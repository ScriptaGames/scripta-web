...
(function () {
    ...
    angular.module('leaderboard', ['ordinal']).directive('plLeaderboard', leaderboard).factory('leaderboardservice', leaderboardservice);
    ...
    function leaderboardservice($http) {
        ...
        function getLeaders() {
            return $http.get(serviceUrl + '?limit=1000', {
                headers: {
                    'X-Parse-Application-Id': 'ENGAGE'
                }
            }).then(getLeadersComplete).catch(getLeadersFailed);

            function getLeadersComplete(response) {
                var leaders = _(response.data.results).sortBy('score').reverse().uniqBy('name').take(10).value();
    ...
    function LeaderboardController($scope, $interval, leaderboardservice) {
        ...
        function getLeaders() {
            return leaderboardservice.getLeaders().then(function (data) {
                vm.leaders = data;
                return vm.leaders;
            });
        }
